...
//DETALLES DEL PRODUCTO SELECCIONADO
    ...
    */
    ...
    ngOnInit() {
        this.paramRutas = {
            productID: this.rutaActiva.snapshot.params.id
        };
        this.rutaActiva.params.subscribe((params: Params) => {
            this.paramRutas.productID = params.id;
        });
        this.empresa = this.profileService.getCurrentUser();
        this.inputImage();
        this.tab();
        this.getProducto(this.paramRutas.productID);
        this.getAcomp();
        this.getAdicional();
        this.getCategorias();
        this.getTamano();
    ...
    getProducto(id) {
        ...
        this.productosService.getProducto(id).subscribe((producto: any) => {
            ...
            this.bebidaSelected = [];
            this.saboresSelected = [];
            if (this.producto.sirope != 0) {
                this.acompSelected.push(1);
                let siropes = JSON.parse(this.producto.sirope).ids.split(",");

                for (let sirope of siropes) {
                this.siropeSelected.push(Number(sirope));
                }
            }
            if (this.producto.topping != 0) {
                this.acompSelected.push(2);
                let toppings = JSON.parse(this.producto.topping).ids.split(",");

                for (let topping of toppings) {
                    this.toppingSelected.push(Number(topping));
                }
            }
            if (this.producto.bebida != 0) {
                this.acompSelected.push(3);
                let bebidas = JSON.parse(this.producto.bebida).ids.split(",");

                for (let bebida of bebidas) {
                this.bebidaSelected.push(Number(bebida));
                }
            }

            if (this.producto.tamanos != 0) {
                let tamanos = JSON.parse(this.producto.tamanos);

                for (let tamano of tamanos) {
                this.tamanoSelected.push(Number(tamano.id));
                }
            }

            if (this.producto.add != 0) {
                let adds = this.producto.add.split(",");

                for (let add of adds) {
                this.addSelected.push(Number(add));
                }
            }

            if (this.producto.sabores != 0) {
                let sabores = JSON.parse(this.producto.sabores);

                for (let sabor of sabores) {
                this.saboresSelected.push(Number(sabor.id));
                }
            }
            this.categoriaSelected = this.producto.categoria_product_id;
            this.infoProducto.patchValue({
                nombre: this.producto.nombre,
                descripcion: this.producto.descripcion,
                cat: this.categoriaSelected,
                acompsCond: this.producto.acomp === 1 ? true : false,
                adicionalCond: this.producto.add != 0 ? true : false,
                saboresCond: this.producto.sabores != 0 ? true : false,
                mismoPrecioCond: this.producto.mismo_precio != 0 ? true : false,
                tamanoCond: this.producto.tamanos != 0 ? true : false,
                toGoCond: this.producto.to_go == 0 ? true : false,
                toGo: this.producto.to_go === "1" ? "" : this.producto.to_go,
                toGo_dl: this.producto.to_go$ === "0" ? "" : this.producto.to_go$,
                cantidadAcomp0: this.producto.sirope != 0 ? JSON.parse(this.producto.sirope).cantidad : 0,
                cantidadAcomp1: this.producto.topping != 0 ? JSON.parse(this.producto.topping).cantidad: 0,
                cantidadAcomp2: this.producto.bebida != 0 ? JSON.parse(this.producto.bebida).cantidad: 0,
                acomps: this.acompSelected,
                siropes: this.siropeSelected,
                toppings: this.toppingSelected,
                bebidas: this.bebidaSelected,
                adicionales: this.addSelected,
                sabores: this.saboresSelected,
                tamano: this.tamanoSelected,
                cantidad: this.producto.cantidad === -1 ? "" : this.producto.cantidad,
                ilimitado: this.producto.cantidad === -1 ? true : false,
                publish: this.producto.publish
            });
            this.precioProducto.patchValue({
                precioUnico: this.producto.precio1,
                precioUnico_dl: this.producto.precio1_dl
            });
        },
        err => {
            //SI HUBO UN ERROR EN LA CONSULTA RECARGA LA PAGINA
        ...
        this.infoProducto.get("saboresCond").valueChanges.pipe(distinctUntilChanged()).subscribe(ResSabores => {
            if (!ResSabores) {
                ...
                this.infoProducto.get("mismoPrecioCond").valueChanges.subscribe(mismoPrecio => {
                    if (!mismoPrecio) {
                        if (this.saboresForm.value.length > 0) {
                            precioUnico.disable();
                            precioUnico.setValidators(null);
                            if (this.empresa.modDollar == 1) {
                                precioUnico_dl.disable();
    ...
    inputImage() {
        $("#inputfilePortadaProd input").each(function() {
            // Refs
            var $file = $(this),
            ...
            $file.on("change", function(event) {
                ...
                if (fileName) {
                    $("#portadaCardProd").attr("src", `${tmppath}`);
                    $label
                    .addClass("file-ok")
    ...
    if(this.tamanosForm.value.length > this.infoProducto.value.tamano.length){
        this.tamanosForm.value.forEach((item,index) => {
            if(!this.infoProducto.value.tamano.some(item1 => item.id == item1)){
        ...
        this.infoProducto.value.tamano.forEach((item,index) => {
            if (!this.tamanosForm.value.some(item1 => item == item1.id)) {
                let nombre = this.tamanos.find(element => element.id == item).nombre
    ...
    modificarSabores(){
        if(this.saboresForm.value.length > this.infoProducto.value.sabores.length){
            this.saboresForm.value.forEach((item,index) => {
                if (!this.infoProducto.value.sabores.some(item1 => item.id == item1)) {
            ...
            this.infoProducto.value.sabores.forEach((item,index) => {
                if (!this.saboresForm.value.some(item1 => item == item1.id)) {
                    let nombre = this.sabores.find(element => element.id == item).nombre
    ...
    verificarTamanos(){
        let empresaTamanos = JSON.parse(this.producto.tamanos)
        if (empresaTamanos != 0) {
            empresaTamanos.forEach((item,index)=>{
                let nombre = this.tamanos.find(element => element.id == item.id).nombre
                if (this.empresa.modDollar == 1) {
                    let tamanosFormGoup = this.fb.group({
                        id: item.id,
                        nombre: nombre,
                        precio: new FormControl(this.cambioABss(item.precio), [Validators.required, Validators.pattern('^[0-9.]+')])
                    })
                    let tamanosFormGoup1 = this.fb.group({
                        id: item.id,
                        nombre: nombre,
                        precio: new FormControl(item.precio, [Validators.required, Validators.pattern('^[0-9.]+')])
                    })
                    this.tamanosForm.push(tamanosFormGoup);
                    this.tamanos_dl.push(tamanosFormGoup1);
                    }else{
    ...
    verificarSabores(){
        let empresaSabores = JSON.parse(this.producto.sabores)
        if (empresaSabores != 0) {
            empresaSabores.forEach((item,index)=>{
            let nombre = this.sabores.find(element => element.id == item.id).nombre
            if (this.empresa.modDollar == 1) {
                let saboresFormGoup = this.fb.group({
                    id: item.id,
                    nombre: nombre,
    ...
    /**
     * VERIFICANDO DATOS Y CREANDO ARRAY
     */
    editarProducto() {
        //VARIABLES
        ...
        if(this.infoProducto.value.saboresCond){
            ...
            for(let i =0;i<todosSabores.length;i++){
                if (this.infoProducto.value.mismoPrecioCond) {
                    if (i == (this.infoProducto.value.sabores.length - 1)) {
                        ids += todosSabores[i];
                    } else {
                        ids += todosSabores[i] + ',';
                    }
    ...
    saveImgProducto() {
        if (this.fileType != "") {
            ...
            this.imagePortada = {
                filename: this.empresa.ruta + "-" + this.paramRutas.productID + "-" + this.nameOfFile,
                filetype: this.typeImage,
                value: valImgPortada
            };
            this.productosService.setImageProducto(this.imagePortada, this.producto.files_id).subscribe((res: any) => {
                if (res.message == "ok") {
                    this.guardarDatos();
                } else {
                    Swal.fire({
     ...
     */
    guardarDatos() {
        this.productosService.updateProducto(this.producto.id, this.agregar).subscribe((add: any) => {
            if (add.message === "ok") {
                ...
                Swal.fire({
                    position: "center",
                    icon: "success",
                    title: "Se ha editado correctamente el producto",
